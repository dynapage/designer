...
export default function Teams() {
  ...
  const processRowUpdate = async newRow => {
    try {
      ...
      if (selectedTeam == 0) {
        setSnackbar({
          children: "Admins team can not be modified",
          severity: "error",
        })
      } else {
        if (newRow.teamName.trim().length > 16) {
          setSnackbar({
            children: "Team Name must be less than 17 chars long",
            severity: "error",
          })
          return newRow
        }
        if (newRow.teamName.trim().length < 4) {
          setSnackbar({
            children: "Team Name must be more than 3 chars long",
            severity: "error",
          })
          return newRow
        }
        try {
          ...
          dispatch(setTeamList(res))
          setSnackbar({
            children: "Team updated successfully",
            severity: "info",
          })
        } catch (err) {
          alert("@0150--" + err)
