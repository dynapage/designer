...
function EditToolbar(props) {
  ...
  const handleClick = () => {
    ...
    setRows(oldRows => [
      ...oldRows,
      { _id: id, username: "", userId: "", userpwd: randStr(12), isNew: true },
    ])
...
export default function UsersGrid({ selectedteam }) {
  ...
  const processRowUpdate = async newRow => {
    ...
    const selectedItem = dbApp.teams.findIndex(
      ({ _id }) => _id === selectedteam._id
    )
    try {
      if (newRow.userId.trim() === "") {
        setSnackbar({
          children: "User email cannot be empty",
          severity: "error",
        })
        return newRow
      }
      if (newRow.userId.trim().length < 7) {
        setSnackbar({
          children: "User email must be more than 6 chars long",
          severity: "error",
        })
        return newRow
      }
      if (newRow.userId.trim().length > 38) {
        setSnackbar({
          children: "User email must be less than 38 chars long",
          severity: "error",
        })
        return newRow
      }
      ...
      if (newRow.username.trim() === "") {
        setSnackbar({
          children: "User name cannot be empty",
          severity: "error",
        })
        return newRow
      }
      ...
      if (newRow.username.trim().length < 7) {
        setSnackbar({
          children: "User name must be more than 6 chars long",
          severity: "error",
        })
        return newRow
      }
      if (newRow.username.trim().length > 38) {
        setSnackbar({
          children: "User name must be less than 38 chars long",
          severity: "error",
        })
        return newRow
      }
      ...
      if (newRow.userpwd.trim() === "") {
        setSnackbar({
          children: "User pssword cannot be empty",
          severity: "error",
        })
        return newRow
      }
      ...
      if (newRow.userpwd.trim().length < 8) {
        setSnackbar({
          children: "User pssword must be between 8 to 14 chars long",
          severity: "error",
        })
        return newRow
      }
      ...
      if (newRow.isNew) {
        ...
        setSnackbar({
          children: "Team user created successfully",
          severity: "info",
        })
        return newRow
      } else {
        ...
        const res = await applicationApi.updateUser(
          selectedApp._id,
          selectedteam._id,
          userId,
          newRow
        ...
        dispatch(setApplication(res))
        setSnackbar({
          children: "Team user updated successfully",
          severity: "info",
        })
        return newRow
  ...
  const handleDeleteClick = id => async () => {
    if (confirm("Are you sure you want to delete this user?") == true) {
      const res = await applicationApi.deleteUser(
        selectedApp._id,
        selectedteam._id,
        id
      )
      ...
      },
      {
        ...
        renderCell: ({ value, index }) => {
          return (
            ...
            >
              {!hidepass.show && hidepass.val != value
                ? "Show password"
                : value}
            </Button>
          )
  ...
  return (
    <div style={{ width: "100%" }}>
      <Divider></Divider>
      <center>
        <h3>{selectedteam.teamName} team users</h3>
      </center>
      {rows ? (
        <Box>
          <DataGrid
