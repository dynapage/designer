...
function EditToolbar(props) {
    ...
    const { selectedCellParams, cellMode, cellModesModel, setCellModesModel, setOpenNewAppDrawer, setShowColumns, selectedApp, selectedRow } = props;
    ...
    const dropTable = async () => {
        try {
            ...
            if (deleteTableConfirmationCounter < 2) {
                const res = await tableApi.delete(selectedApp._id, id, selectedApp.dbname, selectedRow.name);
                ...
                if (res.name) {
                    ...
                    setSnackbar({ children: "Error occurred while trying to delete table", severity: 'warning' })
                }
    ...
    return (
        ...
        > <Stack direction="row" spacing={1}>
                ...
                <Button disabled={selectedApp.length == 0} size="small" onClick={handleNewTable} variant="outlined" sx={{ ml: 1 }}>
                    Add a table
                </Button>
                <Button size="small"
                    onClick={dropTable}
                    ...
                    sx={{ ml: 1 }}
                >
                    {deleteTableConfirmationCounter > 1 ? "Delete Table" : "Delete! Sure?"}
                </Button>
                <Button size="small"
        ...
        <>
            <Box>
                <Box sx={{ padding: '10px 50px' }}>
                    <PagesHeader name={selectedApp.name} sitename={selectedApp.sitename}></PagesHeader>
                    <Divider></Divider>
                    <center><h3>Site Tables</h3></center>
                    <DataGrid
                        sx={{
                    ...
                    {selectedRow.columns && showColumns ?
                        <ColumnGrid initialState={showColumns} selectedApp={selectedApp} selectedTable={selectedRow} closeOpenFlag={setShowColumns}></ColumnGrid>
                        :
                        null}
                    <TableDrawer initialState={childNewAppDrawer} closeOpenFlag={setOpenNewAppDrawer} selectedApp={selectedApp} showcolumnForm={false}></TableDrawer>
                    {/* <p></p>
...
const columns = [
    ...
    { headerName: 'Created', field: 'createdAt', type: 'date', valueGetter: ({ value }) => value && new Date(value), editable: false },
];
