...
function EditToolbar(props) {
  ...
  const handleFormEdit = () => {
    ...
    navigate(`/sections/${application._id}`, {
      state: {
        appId: application._id,
        appName: application.name,
        appSiteName: application.sitename,
        selectedTable: { ...selectedTable },
        selectedForm: { ...selectedRow.row },
        stageId: 4,
      },
    })
  }
  ...
  const processRowUpdate = async newRow => {
    ...
    if (newRow.isNew) {
      if (newRow.label == "") {
        setSnackbar({
          children:
            "No form title provided, saving a new form operation failed",
          severity: "error",
        })
      ...
      if (!newRow.formSchemaRef) {
        setSnackbar({
          children: "Site table is not selected",
          severity: "error",
        })
      ...
      if (rowToInsert.name.trim() === "") {
        setSnackbar({
          children: "No form name provided, saving a new form operation failed",
          severity: "error",
        })
        return newRow
      }
      if (rowToInsert.label.trim() === "") {
        setSnackbar({
          children:
            "No form caption provided, saving a new form operation failed",
          severity: "error",
        })
        return newRow
      }
      if (rowToInsert.label.trim().length > 20) {
        setSnackbar({
          children: "Maximum form name size is 20 characters",
          severity: "error",
        })
        return newRow
      }
      if (rowToInsert.label.trim().length < 4) {
        setSnackbar({
          children: "Minimum form name size is 4 characters",
          severity: "error",
        })
        return newRow
      }
    ...
    <>
      <Box>
        <Box sx={{ padding: "10px 50px" }}>
          ...
          <Divider></Divider>
          <center>
            <h3>Dynapage Forms</h3>
          </center>
          {apptables && apptables.length == 0 ? (
            <center>
              <p>No site tables found, create site tables and columns first</p>
            </center>
