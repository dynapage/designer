...
import { JSONToTree } from '../helpers/index';
...
const addNode = (array, index, payload) => {
  ...
  const result = [...array];
  const addedNode = produce(payload, (draft) => {
...
export const sections = (prevState = { ...defaultState, data: [] },{ type, payload }) => {
  ...
  switch (type) {
    case $.ADD_SECTION_NODE:
      return produce(draftState, performModification(payload, addNode)).state;
    case $.REMOVE_SECTION_NODE:
      return produce(draftState, performModification(payload, removeNode))
        .state;
    case $.MOVE_SECTION_NODE:
      return produce(draftState, performModification(payload, moveNode)).state;
    case $.EDIT_PROPERTIES:
      return produce(draftState, (draft) => {
        let current = draft.state.data;
        payload.path.forEach((pathId) => {
          const originalData = original(current);
          const index = (originalData.children || originalData).findIndex(
            ({ id }) => id === pathId
          );
